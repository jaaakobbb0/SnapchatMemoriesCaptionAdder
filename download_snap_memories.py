...
def get_sid_from_url(url):
    """Extract SID (unique snap ID) from Snapchat URL query parameters."""
    parsed = urlparse(url)
    params = parse_qs(parsed.query)
    return params.get("sid", ["unknown_sid"])[0]
...
def extract_zip(zip_path, date_tag):
    """Extract and rename contents of a ZIP file. Returns True on success."""
    try:
        with zipfile.ZipFile(zip_path, "r") as z:
            for member in z.infolist():
                extracted_name = f"{date_tag}_{os.path.basename(member.filename)}"
...
def download_item(item, index):
    ...
    sid = get_sid_from_url(url)  # <-- Unique per media
    filename_base = f"{date_tag}_{sid}"
    ...
    try:
        ...
        if ext.lower() == ".zip":
            ...
            if extract_zip(filepath, date_tag):
                os.remove(filepath)
                print(f"🗑️ Deleted ZIP: {os.path.basename(filepath)}")
            else:
                print(f"⚠️ Extraction failed; ZIP retained: {os.path.basename(filepath)}")
            return f"[{index}] 📦 Processed ZIP for SID {sid}"
        else:
